...
export function ComicsHeader({ email }: { email?: string | null }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <ComicsHeaderBar email={email} />
    </Suspense>
  );
}

function ComicsHeaderBar({ email: _email }: { email?: string | null }) {
  void _email;
  ...
  const searchActive = searchOpen || pathname.startsWith("/comics/search");
  const libraryFilter = searchParams.get("filter");
  const onComicsHome =
    pathname === "/comics" && (!libraryFilter || libraryFilter === "all");
  const onComicsShelf = pathname === "/comics" && libraryFilter === "shelf";
  ...
  const menuItems = [
    ...
    {
      href: "/comics?filter=shelf",
      label: "Biblioteca",
      icon: Library,
      active: onComicsShelf,
    },
    {
      href: "/comics/stats",
      label: "Estadísticas",
      icon: BarChart3,
      active: pathname.startsWith("/comics/stats"),
    },
  ];
      ...
      <div className="mx-auto flex h-14 w-full min-w-0 max-w-6xl items-center justify-between gap-2 px-4 sm:gap-3 sm:px-6">
        <div className="flex min-w-0 items-center gap-2 sm:gap-6">
          ...
          >
            ...
            <span className="truncate">Cómics</span>
          </Link>
