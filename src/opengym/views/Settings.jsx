  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in via Callejón Diagon — data syncs to your account.')} />
        <Row icon="chevron" iconTint="var(--acc)" title="Volver a Callejón Diagon" accessory="chevron"
          onClick={() => { window.location.assign('/home') }} />
        <Row icon="signOut" iconTint="var(--red)" title={t('Sign out')} danger onClick={() => confirmSheet({ title: t('Sign out?'), message: t('Your data is synced to your profile first, then cleared from this device.'), confirmText: t('Sign out'), danger: true, onConfirm: () => { signOut() } })} />
      </> : webauthnOK() ? <>
