...
export const useStore = create((set, get) => {
  ...
  return {
    ...
    async signOut() {
      ...
      clearLocalSession()
      // Leave Deporte 2 and return to Callejón Diagon (do not end the Supabase session).
      if (typeof window !== 'undefined') window.location.assign('/home')
    },

    // In this embed there is no separate passkey session version — same as a local leave.
    async signOutAll() {
      await get().pushState()
      try { await api('/api/logout/all', { method: 'POST', body: '{}' }) } catch (e) { /* */ }
      clearLocalSession()
      if (typeof window !== 'undefined') window.location.assign('/home')
    },
